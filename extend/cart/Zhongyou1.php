...
class Zhongyou {
	...
	private function canInsert($baoguo,$item){
		if ($baoguo['totalNumber']>=$this->maxNumber) {//总数不能超过10个
			return false;
		}
	...
	//判断与当前包裹中的商品能否混寄
	private function canHybrid($baoguo,$type){
		if ($type['no']) {
			foreach ($baoguo['goods'] as $key => $value) {
				if (in_array($value['typeID'],$type['no'])) {
					return false;
					break;
				}
			}
