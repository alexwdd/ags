...
class Finance extends User
{
	...
	{
        $map['memberID'] = $this->user['id'];
        $list = db('Jiesuan')->where($map)->order('id desc')->paginate(24,true,['query'=>request()->param()])->each(function($item, $key){
	...
	public function local(){
        ...
        foreach ($order as $key => $value) {
            ...
            $temp = db("OrderDetail")->field('number,'.$fencheng.' as jiangjin')->where($map)->select();
            foreach ($temp as $k => $v) {
